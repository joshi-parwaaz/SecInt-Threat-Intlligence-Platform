...
const Dashboard = () => {
  ...
  // Detect demo mode from backend root endpoint
  const checkDemoMode = async () => {
    try {
      ...
      // If backend is unreachable, silently skip
    }
  };
  ...
  const fetchTopThreats = async () => {
    try {
      const response = await fetch(API_BASE + '/api/reports/top-threats?limit=10');
      const data = await response.json();
  ...
  const fetchBlocklist = async () => {
    try {
      const response = await fetch(API_BASE + '/api/reports/blocklist');
      const data = await response.json();
      setBlocklist(data);
    } catch (err) {
  ...
  const pollIngestionStatus = async () => {
    const checkStatus = async () => {
      try {
        const response = await fetch(API_BASE + '/api/ingestion/status');
        const data = await response.json();
  ...
  return (
    <div className="min-h-screen bg-black text-white p-8">
      {/* Demo Mode Banner */}
      {demoMode && (
        <div className="mb-6 rounded-lg border border-yellow-500/40 bg-yellow-500/10 px-5 py-3 flex items-center gap-3 text-sm">
          <span className="text-yellow-400 text-lg">🧪</span>
          <div>
            <span className="font-semibold text-yellow-300">Demo Mode — </span>
            <span className="text-yellow-200/80">
              Showing {stats?.total_iocs || 0} seeded IOCs. Add API keys (OTX, VirusTotal, AbuseIPDB) to Render env vars to enable live threat feeds and auto-refresh.
            </span>
          </div>
        </div>
      )}
